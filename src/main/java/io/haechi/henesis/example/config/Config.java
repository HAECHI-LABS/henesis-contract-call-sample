...
public class Config {
     ...
     */
    @Bean
    public IERC20 ethToken() {
        try {
            ECKeyPair dummyKeyPair = Keys.createEcKeyPair();
            return IERC20.load(
                    this.henesisApiProperties.getErcTokenAddress(), // ETH-EVT: "0x0d4c27c49906208fbd9a9f3a43c63ccbd089f3bf"
                    this.web3j(),
    ...
    @Bean
    public IERC20 klayToken() {
        try {
            ECKeyPair dummyKeyPair = Keys.createEcKeyPair();
            return IERC20.load(
                    this.henesisApiProperties.getKctTokenAddress(), // KLAY-EVT: "0x5ff1e48b489c67366f936ed82978e913debd7ad8"
                    this.web3j(),
                    Credentials.create(dummyKeyPair),
                    new DefaultGasProvider()
            );
    ...
    @Bean
    public TokenSupplier<IERC20> functionSupplier() {
        Map<Blockchain, IERC20> token = new HashMap<>();
        token.put(Blockchain.ETH, this.ethToken());
        token.put(Blockchain.KLAY, this.klayToken());
        return new TokenSupplier<>(token);
    }
